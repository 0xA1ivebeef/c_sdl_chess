...
uint64_t get_sliding_piece_attack_moves(int p, int sq, int piece, uint64_t* occ)
{
    uint64_t res = 0;

    int startindex = (piece == 2 ? 4 : 0);
    int endindex = (piece == 3 ? 4 : 8);
...
uint64_t get_attack_moves_bitmask(int p, int sq, int piece, uint64_t* occ)
{
	piece %= 6;
	switch(piece)
	{
		case 0:
			return get_pawn_attack_moves(p, sq, occ[!p]);
		case 1:
			return get_knight_attack_moves(p, sq, occ);
		case 2:
		case 3:
		case 4:
			return get_sliding_piece_attack_moves(p, sq, piece, occ);
		case 5:
...
uint64_t resolve_attack_bb(int p, uint64_t bb, int piece, uint64_t* occ)
{
	uint64_t res = 0;
	while(bb)
	{
        int sq = __builtin_ctzll(bb);
		res |= get_attack_moves_bitmask(p, sq, piece, occ);
        bb &= bb - 1;
	}
	return res;
}
