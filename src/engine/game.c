...
int setup(GameContext* game, SDL_Renderer* renderer)
{
    ...
    load_bitmasks();
    generate_legal_moves(game->game_flags[0], game->bitboards, game->occupancy_bitboards, game->game_flags, game->legal_moves, 0);
...
void update(GameContext* game)
{
    ...
    update_occupancy_bitboards(game->bitboards, game->occupancy_bitboards);

    // generate enemy atk_bb
    uint64_t atk_bb = get_attack_bitboard(!(game->game_flags[0]), game->bitboards, game->occupancy, game->game_flags);
    ...
    filter_moves(game->game_flags[0], game->bitboards, game->occupancy_bitboards, game->game_flags, game->legal_moves, atk_bb);
