...
// table of move values for fen strings in order
const int FEN_STRING_MOVE_COUNT_DEPTH_5[FEN_STRING_COUNT] = 
{
    4865609,
    193690690,
    674624,
...
const char* FEN_STRINGS[FEN_STRING_COUNT] = 
{
    "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",
    // "r3k2r/p1ppqpb1/1n2pnp1/3PN3/1p2P3/2N2Q1p/PPPBbPPP/R3K2R w KQkq - 1 0",
...
void substr(const char* fen_string, char* buffer, int start, int buffer_size)
{
    int i = 0;
...
void resolve_game_flags(Position* pos, const char* fen_string, int fen_string_index)
{
    int buffer_size = strlen(fen_string) - fen_string_index + 1;
    char buff[buffer_size];
    substr(fen_string, buff, fen_string_index, buffer_size);
    int index = 0;
...
void load_fen_string(Position* pos, int index)
{
    ...
    const char* fen_string = FEN_STRINGS[index];
    ...
    while((i < strlen(fen_string)) && c != ' ')
    {
        ...
        {
            piece = (char_to_int(to_lower(c)) + (is_upper(c) ? 6 : 0));
            pos->bb[piece] |= (1ULL << (row * 8 + col));
            col++;
        }
        ++i;
    }

    resolve_game_flags(pos, fen_string, i);
}
